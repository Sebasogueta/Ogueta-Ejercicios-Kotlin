...
@Composable
fun Project107(navController: NavController) {
    Column(modifier = Modifier.padding(20.dp)) {
        Text(
            text = "Project 107: Enter the size of the array and assign a value to every element",
            ...
            onClick = {
                if (arraySize.isNotEmpty()) {
                    ...
                    if (numbers <= arraySize.toInt()) {
                        ...
                        numValue = ""
                        if (numbers > arraySize.toInt()) {
                            val sum = numArray?.sum() ?: 0 // ?.sum() if numArray is null, sum returns null // ?:0 if sum() returns null, the value sum will be 0
                            outcome = "Array creation completed. Sum of array elements: $sum"
                            arraySize = ""
                            numbers = 1
                            numArray = null
                            arrayEnter = true
